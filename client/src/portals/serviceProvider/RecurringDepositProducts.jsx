...
import { getRecurringDepositProducts, createRecurringDepositProduct, updateRecurringDepositProduct } from '../../api/serviceProviderApi';
...
const fields = [
  ...
  { name: 'recurringDepositFrequency', label: 'Recurring Deposit Frequency', type: 'number', defaultValue: '1', required: true },
  { name: 'recurringDepositFrequencyTypeId', label: 'Recurring Deposit Frequency Type', required: true, options: [{ value: 0, label: 'Days' }, { value: 1, label: 'Weeks' }, { value: 2, label: 'Months' }, { value: 3, label: 'Years' }] },
  { name: 'minDepositAmount', label: 'Min Deposit Amount', type: 'number' },
];

export default function RecurringDepositProducts() {
  ...
  const handleSubmit = async (values) => {
    ...
    if (dialog.edit) await updateRecurringDepositProduct(dialog.edit.id, payload);
    else await createRecurringDepositProduct(payload);
    load();
  };
  ...
  return (
    <Box>
      <Typography variant="h5" gutterBottom>Recurring Deposit Products</Typography>
      {pageError && <ErrorAlert error={pageError} onClose={() => setPageError(null)} />}
      <DataTable rows={rows} columns={columns} loading={loading} onAdd={() => setDialog({ open: true, edit: null })} addLabel="New Recurring Deposit Product" />
      <FormDialog open={dialog.open} title={dialog.edit ? 'Edit Recurring Deposit Product' : 'Create Recurring Deposit Product'} fields={fields} initialValues={dialog.edit} onSubmit={handleSubmit} onClose={() => setDialog({ open: false, edit: null })} />
    </Box>
  );
}
